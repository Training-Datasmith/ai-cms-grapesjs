...
<div id="seo" class="item-seo tab-pane fade" role="tablist" aria-labelledby="seo">
	...
	<div id="item-text-group"
		data-translate="<?php 
echo $enc->attr($this->config('admin/jqadm/api/translate', []));
?>"
		data-data="<?php 
echo $enc->attr($this->get('seoData', []));
?>"
		data-siteid="<?php 
...
?>" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
										title="<?php 
echo $enc->attr($this->translate('admin', 'Translate text'));
?>">
									</a>
									<div class="dropdown-menu dropdown-menu-end" v-bind:aria-labelledby="'translate-menu-' + index">
										<?php 
foreach (['de', 'en', 'es', 'fr', 'it', 'nl', 'pl', 'pt', 'ru'] as $lang) {
    ?>
											<a class="dropdown-item" href="#" v-on:click="translate(index, `<?php 
    echo strtoupper($lang);
    ?>`)"><?php 
    echo $enc->html($this->translate('language', $lang));
    ?></a>
										<?php 
}
?>
...
?>"
						...
						<div v-bind:id="'item-text-group-data-' + index" v-bind:class="element['_show'] ? 'show' : 'collapsed'"
							...
							<input type="hidden" v-model="element['text.id']"
								v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'text.id']));
?>`.replace('_idx_', index)">
...
?>"
										...
										<textarea v-show="!element['_show']" class="form-control item-content" readonly
											v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'text.content']));
?>`.replace('_idx_', index)"
...
?>"
											v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'text.status']));
?>`.replace('_idx_', index)"
...
if (!($languages = $this->get('pageLangItems', map()))->count() !== 1) {
    ...
    ?>"
												v-bind:name="`<?php 
    echo $enc->js($this->formparam(['seo', '_idx_', 'text.languageid']));
    ?>`.replace('_idx_', index)"
												v-bind:text="`<?php 
...
?>
								...
								<?php 
if (($seo_types = $this->get('seoTypes', map()))->count() !== 1) {
    ?>
									<div class="form-group row mandatory">
										<label class="col-sm-4 form-control-label help"><?php 
    ...
    ?>"
												v-bind:items="<?php 
    echo $enc->attr($seo_types->col('text.type.label', 'text.type.code')->to_array());
    ?>"
												v-bind:name="`<?php 
    echo $enc->js($this->formparam(['seo', '_idx_', 'text.type']));
    ?>`.replace('_idx_', index)"
												v-bind:text="`<?php 
    echo $enc->js($this->translate('admin', 'Please select'));
    ?>`"
										...
										<div class="col-sm-12 form-text text-muted help-text">
											<?php 
    echo $enc->html($this->translate('admin', 'Types for additional texts like per one lb/kg or per month'));
    ?>
										</div>
									</div>
								<?php 
    ...
    ?>`.replace('_idx_', index)"
										value="<?php 
    echo $enc->attr($seo_types->get_code()->first());
    ?>">
								<?php 
}
?>
								...
								<div class="form-group row optional">
										<label class="col-sm-4 form-control-label help"><?php 
echo $enc->html($this->translate('admin', 'Label'));
?></label>
									<div class="col-sm-8">
										<input class="form-control item-label" type="text" tabindex="<?php 
echo $this->get('tabindex');
?>"
											v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'text.label']));
?>`.replace('_idx_', index)"
											placeholder="<?php 
echo $enc->attr($this->translate('admin', 'Label'));
?>"
											v-bind:readonly="!can('change', index)"
									...
									<div class="col-sm-12 form-text text-muted help-text">
										<?php 
echo $enc->html($this->translate('admin', 'Description of the text content if it\'s in a foreign language'));
?>
									</div>
								</div>
...
?></span>
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
								...
								<?php 
if (!($list_types = $this->get('seoListTypes', map()))->count() !== 1) {
    ?>
									<div class="form-group row mandatory">
										<label class="col-sm-4 form-control-label help"><?php 
    ...
    ?>"
												v-bind:name="`<?php 
    echo $enc->js($this->formparam(['seo', '_idx_', 'cms.lists.type']));
    ?>`.replace('_idx_', index)"
												v-bind:text="`<?php 
    echo $enc->js($this->translate('admin', 'Please select'));
    ?>`"
...
?>"
											v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'cms.lists.datestart']));
?>`.replace('_idx_', index)"
											placeholder="<?php 
...
?>"
											v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'cms.lists.dateend']));
?>`.replace('_idx_', index)"
											placeholder="<?php 
echo $enc->attr($this->translate('admin', 'YYYY-MM-DD hh:mm:ss (optional)'));
?>"
...
?>
							...
							<div v-show="element['_ext']" class="col-xl-6 secondary">
								<config-table tabindex="<?php 
echo $enc->js($this->get('tabindex'));
?>"
									v-bind:keys="<?php 
echo $enc->attr($this->config('admin/jqadm/cms/item/seo/config/suggest', []));
?>"
									v-bind:name="`<?php 
echo $enc->js($this->formparam(['seo', '_idx_', 'config', '_pos_', '_key_']));
?>`"
									v-bind:index="index"
									v-bind:readonly="!can('change', index)"
