 ...
 * @subpackage JsonApi
 */
namespace Aimeos\Client\Json_Api\Cms;
...
class Standard extends \Aimeos\Client\Json_Api\Base implements \Aimeos\Client\Json_Api\Iface
{
    ...
    protected function get_items(\Aimeos\Base\View\Iface $view, Server_Request_Interface $request, Response_Interface $response): \Psr\Http\Message\Response_Interface
    {
        ...
        $items = \Aimeos\Controller\Frontend::create($this->context(), 'cms')->slice($view->param('page/offset', 0), $view->param('page/limit', 25))->sort($view->param('sort'))->parse($view->param('filter', []))->uses($ref)->search($total);
        $view->items = $items;
        $view->total = $total;
        return $response;
    }
